...
14. **`audience` 有四档，`signed`（报名可见）和另外三档不是一类**：它不看角色，看这个人有没有报名，所以可见性第一次依赖具体玩家的数据。判定函数因此分两条 —— `activityVisibleTo(a, role, signedIds)` 管护照可见和计分；`activityRoleVisibleTo(a, role)` 只看角色，**扫码落地页和报名接口必须用它**，否则没报名的人看不到也报不进来，这一档就成了一把锁死的门。同工对 `signed` 一律可见（要核对名单、要盖章）。
...
17. 翻页热区是按**元素自己的盒子**算左右四分之一，而各类页面的盒子宽度不同（横屏下身份页那类 `book-flip` 是全屏 812 宽，欢迎页那类只有居中 430 宽）。换算成屏幕坐标时别想当然。横屏下窄页两侧各有约 190px 点了没反应，是已知的体验缺口，尚未处理。
