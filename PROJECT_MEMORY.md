...
3. （2026-09-20 已解决：活动清单加了 `_activitiesRev` 乐观锁，冲突返回 409 由前端让人选。）
...
**2026-09-20 找回「重置密码」的入口** —— 已提交，前端构建已上线（无服务端改动）。
- 现象：总控台里找不到重置密码。查下来 `manualOpen` 这个 state 从来没有被置成 true ——
  弹层组件 `PlayerSheet`、`resetPinPicked()` 和 `/api/admin/reset-pin` 都在，只是某次改版把
  那个按钮删掉了，整条路径点不到。
- 在「👥 用户」面板的按钮排（排行榜 / 导出 / 备份 / ↻）里加回「🔑 重置密码」。
- 验证：隔离实例里点按钮 → 勾人 → 确认，`/api/restore` 用 3927 通过（200）、原密码被拒（403）。
- 按用户要求又在**每个人的详情弹层**里加了单独的「🔑 重置密码为 3927」（删除按钮上方），
  现场「某某忘了密码」直接点开他就能按，不用回去批量勾选。批量那个入口保留。
  同样验证过：重置后 3927 能找回（200）、原密码被拒（403）。

**2026-09-20 画板缩放把手改成八个** —— 已提交，前端构建已上线（无服务端改动）。
- 原来只有右下角一个把手，只能往右下拉；现在四角 + 四条边中点共八个。
- 四角同时改两边，边中点只改一条边；抓北边/西边时对边钉住不动
  （`drag()` 的 mode 从 `'size'` 变成 `'size:<dir>'`，dir 是 nw/n/ne/e/se/s/sw/w）。
- 吸附跟着正在动的那条边算，不再固定按右边/下边。
- 触屏上给把手垫了一圈 9px 的透明热区（`@media (pointer: coarse)`），手指好按。
- 验证：桌面宽度实测八个把手位置正确；拖左上角时右下角不动（left +7.2 / width −7.2），
  拖右边只变宽、拖下边只变高；`npm test` 全过（27 / 257 / 11 / 33）。

**2026-09-20 点开的大图不再顶出屏幕** —— 已提交，前端构建已上线（无服务端改动）。
- 症状：手机横屏点开照片，只看到上半张，下面被截断。
- 原因：`.photo-gallery-modal__viewer` 是 grid，行高默认按内容算 —— 图片用原始高度把行撑开，
  这时 img 上的 `max-height: 100%` 成了循环引用被浏览器忽略（844×390 下实测图高 1125px、
  容器只有 329px）。
- 改法：viewer 明确写 `grid-template: minmax(0,1fr) / minmax(0,1fr)`，img 改成
  `width/height: 100% + object-fit: contain`，尺寸不再由图片反向决定。
- 图库弹层的大图用的是同一套类，一并修好。
- 验证：844×390 横屏，竖构图照片（带上下色带）完整落在 329px 的容器里，不再溢出。

**2026-09-20 图库缩略图改成整张显示** —— 已提交，前端构建已上线（无服务端改动）。
- 症状：手机横屏看照片页，每张照片只剩中间一条。
- 原因：`.visa-gallery__grid img` 是 `object-fit: cover`，竖构图照片放进横格子会被裁掉上下；
  横屏时格子更扁，裁得更狠。弹层里的缩略图同理。
- 改成 `contain`：宁可留边也不切人头。点开的大图本来就是 contain，没动。
- 单独的「图片/配图」块仍然默认 cover（画板里可以逐块选「完整显示」），没跟着改。
- 验证：844×390 横屏，用带上下色带的竖图核对，红蓝两端都在。

**2026-09-20 活动清单加乐观锁（两个人同时编辑不再互相覆盖）** —— 已提交并上线（重启核对见本条末尾）。
- 原来 `POST /api/admin/activities` 是整份替换、没有任何版本控制：两个人同时开着总控台/画板，
  后保存的会把先保存的整个盖掉，而且自己毫不知情（这就是 6.2 里那条「没有冲突控制」的已知问题）。
- 服务端：`setActivities()` 每次把 `_activitiesRev` +1；`/api/config` 和保存响应都带 `activitiesRev`；
  保存时可带 `rev`，对不上返回 **409 + `conflict: true` + 当前最新的清单和版本号**。
  不带 `rev` 照旧放行（旧前端缓存、脚本、测试还在用）。
- 前端：画板和活动详情页都记住「打开时的 rev」并在保存时带上。
  撞上冲突时不自动决定 —— 画板弹选择框（载入最新 / 用我的覆盖），详情页停掉自动保存并在顶部
  显示一条黄色提示条，两个按钮同样含义。画板里还有一条「别人刚保存过这一场」的提前提示。
- 「用我的覆盖」是以**服务端最新那份清单**为底、只把这一场换成自己的，
  否则会顺手把别人新建的活动抹掉（已验证：B 新建的活动在 A 覆盖后仍在）。
- 验证：`npm test` 全过（27 / 257 / 11 / 33，新增 7 项乐观锁检查）；
  浏览器里真开两边模拟：A 改→B 存→A 自动保存被拒并出提示条；两个按钮都按预期落地。
- 部署：重启前备份 `server/data/pre-optlock-2026-09-20.db`；旧进程 3886 → 新进程经
  `scripts/start-local-server.sh` 启动，推送公钥指纹不变，30 用户 / 21 报名 / 60 事件 / 6 场活动，
  `/api/config` 已带 `activitiesRev`（正式库起点 0，下次保存起递增），三个公网入口正常，隧道未动。

**2026-09-20 护照按「看得见的高度」排版（横屏被工具栏遮住那一截）** —— 已提交，前端构建已上线。
- 症状：手机横屏看照片页，底下一截被遮住，要往下拽一把才看得全。
- 原因：整屏容器是 `position: fixed; inset: 0; height: auto`，iOS 横屏时 Safari 工具栏压在
  100dvh 里面；`html.book-locked` 又把滚动锁死，所以被遮的部分只能靠「拽一下让工具栏收起」才看到。
- 改法：`PassportBook` 用 `visualViewport.height` 量当前真正可见的高度（键盘弹出那种骤降忽略掉），
  经 `ui.viewportH` → `bookVals.screenHeight` 传给视觉层，容器高度用它，量不到退回 `100dvh`。
- **注意**：`PassportBookView.jsx` 是 `design/convert.py` 生成的，这次手改了容器那一行
  （`height: v.screenHeight`）。下次重新生成设计稿后要把这一行补回去。
- 验证：844×320 / 812×375 横屏与 375×812 竖屏，容器高度都等于可见高度、页面不再溢出；
  `npm test` 全过（27 / 249 / 11 / 33）。真机 iOS 横屏未验证。

**2026-09-20 页签：横滑与拖动排序分开，横屏也分层** —— 已提交，前端构建已上线。
- 两个手势都在同一条横向轨道上会打架。现在手指**按住 0.32 秒**才进入排序（进入时轻震一下），
  按住之前横滑超过 8px 就放手交给浏览器滚动；鼠标仍然按下即可拖。页签 `touch-action` 从
  `none` 改成 `pan-x`，否则浏览器根本不会滚。
- 横屏（`max-height:600px and orientation:landscape`）也改成分层：页签一行、模版按钮一行，
  不再和页名挤成一条横滚长龙；页签条上下各留 4–7px，手指好滑。
- 验证（812×375 与 375×812）：快速横滑不会改顺序；按住再拖会改；轻点照常切页；鼠标拖动照旧。

**2026-09-20 画板头部在手机上重排** —— 已提交，前端构建已上线（无服务端改动）。
- 原来窄屏是「页签 + 一排按钮 + 页名」挤在一行里横向滚动（`max-width: 46vw` + `overflow-x`），
  画布被顶到屏幕外，人看不到自己在排什么版。改成分层：页签一行（可横滑）、
  三个模版按钮 + 删除一行、页名单独一行。
- 窄屏按钮文字收短：「＋ 照片页」→「＋ 照片」，「删除本页」→「删除」（`design__narrow-hide`）；
  ← → 挪页按钮在手机上隐藏 —— 页签本来就能拖着排序。
- 注意这几条规则要写在 `@media (max-width: 720px)` 那一块，第一次误放进了
  `max-height: 600px and (orientation: landscape)`，手机竖屏根本不匹配。
- 验证：375×812 实测头部从 251px 降到 207px，画布完整可见；页签超出时横滑正常。

**2026-09-20 视频上限改成 100MB** —— 已提交并上线（前端构建 + 重启）。
- `VIDEO_MAX` 40MB → 100MB，`express.raw` 的 limit 一并抬到 110mb（留一点头），
  前端 `VIDEO_MAX_BYTES` 和画板说明文字同步。
- 注意：整段视频是一次性读进内存再落盘的，100MB 的片子上传瞬间会占同等内存；
  以后要收更大的，得改成边收边写（流式），不能只调这个数。

**2026-09-20 附加页三个模版、页名不再带编号** —— 已提交并上线（2026-09-20 重启，见本条末尾）。
- 新增「空白页」模版（`kind: 'blank'`，一张白纸、没有预置块），和照片页、总结页并排三个按钮。
- 新建页名去掉序号：「照片页」「活动总结」「空白页」——序号页签上本来就有。
- `addPage` 也改成先算好再 setState（同前面那批 ref 同步化）。
- 验证：`npm test` 全过（27 / 249 / 11 / 33，新增空白页类型保留的检查）；
  隔离实例里三个按钮各加一页并保存，服务端拿到 photo / summary / blank 三种类型。
- 部署（含当天更早的视频块 / 补报名等前端改动）：重启前备份 `server/data/pre-video-blank-2026-09-20.db`；
  旧进程 96515 → 新进程经 `scripts/start-local-server.sh` 启动，打开的仍是项目 `server/data/game.db`，
  推送公钥指纹不变，22 用户 / 13 报名 / 49 事件 / 6 场活动，game/staff/city 三个入口正常，隧道未动。
  `/api/admin/upload/video` 已上线（用非视频数据探测返回 400 而不是 404）。9 个提交已推到
  `origin/passport-checkin` 和 `origin/main`。

**2026-09-20 画板工具条按钮补上文字** —— 已提交，前端构建已上线。
- 吸附按钮写「吸附已开 / 吸附已关」，全屏按钮写「全屏编辑 / 退出全屏」。
- 窄屏原来把这两个按钮的文字 `display:none` 只留一个 🧲 / ⛶ 图标，现在保留文字、
  按钮宽度按内容走（手机 375px 实测两个按钮和撤销/重做同排放得下）。
