...
21. 翻页热区是按**元素自己的盒子**算左右四分之一，而各类页面的盒子宽度不同（横屏下身份页那类 `book-flip` 是全屏 812 宽，欢迎页那类只有居中 430 宽）。换算成屏幕坐标时别想当然。横屏下窄页两侧各有约 190px 点了没反应，是已知的体验缺口，尚未处理。
...
- 做在活动详情的「📣 报名」区：报名名单每人一个「签到」按钮（已签到显示「已签到 ✓」），上方一个「一键全签到（n）」，只给还没签到的人盖，有确认框说明章撤不掉。统计行改成「报名 n 人，已签到 m 人」，并写明没报名就来的人要去「👥 用户」或扫码。
...
- 已知缺口：没报名直接到场的人，仍然只能在「👥 用户」详情里标记或扫码盖章 —— 活动页上没有「搜人并签到」的入口。
