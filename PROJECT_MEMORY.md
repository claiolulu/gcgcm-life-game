...
14. 翻页热区是按**元素自己的盒子**算左右四分之一，而各类页面的盒子宽度不同（横屏下身份页那类 `book-flip` 是全屏 812 宽，欢迎页那类只有居中 430 宽）。换算成屏幕坐标时别想当然。横屏下窄页两侧各有约 190px 点了没反应，是已知的体验缺口，尚未处理。
...
- 追加（同日）：签证页竖屏下滚不动 —— 整页 `rotate(90deg)` 把块内滚动映射成了屏幕横向。补 `useCrossAxisScroll`（见注意点 11）。
  ...
  - 顺带查明：翻页热区按元素自身盒子算，横屏下窄页两侧约 190px 是死区（记为注意点 14，未处理）。
