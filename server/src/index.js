...
app.post('/api/admin/activities', staffAuth('admin'), (req, res) => {
  ...
  /**
   * 纪元 +1：各端的花名册是增量同步的，只带回 updated_at 变过的人。
   *
   * 而分数和场次是**算出来的**，算的时候要看活动清单（删掉的活动不计分，
   * 对这个人角色不可见的活动也不计分）。改一次活动，所有人的数字都可能变，
   * 却没有任何一行 players.updated_at 被动过 —— 于是增量同步一个人都不返回，
   * 各端一直显示改之前的旧分数，刷新也没用，只有重新登录才对。
   *
   * 纪元一变，客户端就知道手里那份作废，整份重拉。花名册就几十个人，
   * 而活动又不是每天改，这点开销换一个「改完立刻对得上」值。
   */
  setSetting('_epoch', epoch() + 1);
